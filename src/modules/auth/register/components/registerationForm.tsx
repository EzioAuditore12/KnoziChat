...
import { registerUserInputs, registerUserValidations } from "../validations";
...
export function RegisterationForm({
	...
	const onSubmit = async (data: registerUserInputs) => {
		...
		let fileProfilePicture = null;
		if (profilePicture) {
			// For React Native, create a proper file object for FormData
			fileProfilePicture = {
				uri: profilePicture,
				type: "image/jpeg", // or determine from the file
				name: "avatar.jpg",
			} as any;
		}
	...
	return (
		<View className="gap-y-1 max-w-md p-2 w-full">
			<Controller
				control={control}
				name="profilePicture"
				render={({ field: { value, onChange } }) => (
					<UploadAvatar className="mx-auto" value={value} onChange={onChange} />
				)}
			/>
